...
let generate_lean_code (prgm : Ast.program) (prg_scopelang : 'm Scopelang.Ast.program) : string =
  ...
  let (scope_code, toplevel_function_code, scope_func_names, all_assertions) = List.fold_left (fun (scopes_acc, topdefs_acc, func_names_acc, assertions_acc) vertex ->
    match vertex with
    | Scopelang.Dependency.Scope (scope_name, _module_opt) -> 
        ...
        | None -> (scopes_acc, topdefs_acc, func_names_acc, assertions_acc))
    | Scopelang.Dependency.Topdef topdef_name ->
        ...
        | Some (tn, topdef_decl) ->
            ...
            (scopes_acc, code :: topdefs_acc, func_names_acc, assertions_acc)
        | None -> (scopes_acc, topdefs_acc, func_names_acc, assertions_acc))
  ) ([], [], [], []) defs_ordering in
...
let () =
  Driver.Plugin.register "lean4-desugared-test" term
    ~doc:
