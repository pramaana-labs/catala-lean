...
def section_2_spouse_death_events_in_window := (fun (spouse_arg : (Optional Individual)) (death_events_arg : (List DeathEvent)) (tax_year_arg : Int) => (match spouse_arg with
  | Optional.Absent _ => (List.filter ((fun (death_event : DeathEvent) => false)) death_events_arg)
  | Optional.Present s => (List.filter ((fun (death_event : DeathEvent) => (decide (((death_event).decedent = s)) && (decide (((Date_en.get_year (death_event).death_date) ≥ (tax_year_arg - (2 : Int)))) && decide (((Date_en.get_year (death_event).death_date) < tax_year_arg)))))) death_events_arg)))

def section_2_spouse_death_year := (fun (spouse_death_events_in_window_arg : (List DeathEvent)) => (if ((spouse_death_events_in_window_arg).length > (0 : Int)) then (Optional.Present (match (List.map ((fun (death_event : DeathEvent) => (Date_en.get_year (death_event).death_date))) spouse_death_events_in_window_arg) with | [] => (fun () => -100029) () | x0 :: xn => List.foldl (fun (max1 : Int) (max2 : Int) => (if (max1 > max2) then max1 else max2)) x0 xn)) else (Optional.Absent ())))

def section_2_most_recent_spouse_death_date := (fun (spouse_death_events_in_window_arg : (List DeathEvent)) (tax_year_arg : Int) => (if ((spouse_death_events_in_window_arg).length > (0 : Int)) then (match (List.map ((fun (death_event : DeathEvent) => (death_event).death_date)) spouse_death_events_in_window_arg) with | [] => (fun () => CatalaRuntime.Date.mk 0 0 0) () | x0 :: xn => List.foldl (fun (max1 : CatalaRuntime.Date) (max2 : CatalaRuntime.Date) => (if (max1 > max2) then max1 else max2)) x0 xn) else (Date_en.of_year_month_day tax_year_arg (1 : Int) (1 : Int))))

def individual_attained_age_65 := (fun (individual_arg : Individual) (birth_events_arg : (List BirthEvent)) (year_end_arg : CatalaRuntime.Date) => (List.foldl ((fun (acc : Bool) (birth_event : BirthEvent) => (decide (acc) || (decide (((birth_event).individual = individual_arg)) && decide ((Date_en.is_old_enough year_end_arg (birth_event).birth_date (CatalaRuntime.Duration.create 65 0 0))))))) false birth_events_arg))
...
def get_taxpayer := (fun (individual_tax_return_arg : IndividualTaxReturn) => (match (individual_tax_return_arg).details with
  | FilingStatusVariant.JointReturn variant => (variant).taxpayer
  | FilingStatusVariant.SurvivingSpouse variant => (variant).taxpayer
...
def get_spouse := (fun (individual_tax_return_arg : IndividualTaxReturn) => (match (individual_tax_return_arg).details with
  | FilingStatusVariant.JointReturn variant => (Optional.Present (variant).spouse)
  | FilingStatusVariant.SurvivingSpouse variant => (Optional.Present (variant).deceased_spouse)
  | FilingStatusVariant.HeadOfHousehold variant => (Optional.Absent ())
  | FilingStatusVariant.Single variant => (Optional.Absent ())
  | FilingStatusVariant.MarriedFilingSeparate variant => (Optional.Present (variant).spouse)))

def extract_spouse_itemization_election := (fun (individual_tax_return_arg : IndividualTaxReturn) => (match (individual_tax_return_arg).details with
  | FilingStatusVariant.JointReturn variant => false
  | FilingStatusVariant.SurvivingSpouse variant => false
  | FilingStatusVariant.HeadOfHousehold variant => false
  | FilingStatusVariant.Single variant => false
  | FilingStatusVariant.MarriedFilingSeparate variant => (variant).spouse_itemization_election))
...
structure IndividualSection151ExemptionsList_Input where
  ...
  individual : Individual
  spouse_personal_exemption_allowed : Bool := (if is_joint_or_surviving_spouse then false else (match spouse with
  | Optional.Absent _ => false
  | Optional.Present s => ((fun (spouse_has_no_income : Bool) => ((fun (spouse_is_not_dependent_of_another : Bool) => (decide (spouse_has_no_income) && decide (spouse_is_not_dependent_of_another))) (!decide ((List.foldl ((fun (acc : Bool) (event : TaxReturnEvent) => (decide (acc) || ((!decide (((event).individual = individual))) && (decide (((event).tax_year = tax_year)) && decide ((List.foldl ((fun (acc : Bool) (dependent : Individual) => (decide (acc) || decide ((dependent = s))))) false (event).dependents))))))) false tax_return_events))))) (!decide ((List.foldl ((fun (acc : Bool) (income_event : IncomeEvent) => (decide (acc) || (decide (((income_event).individual = s)) && (decide (((income_event).tax_year = tax_year)) && decide ((income_event).has_income)))))) false income_events))))))
  individuals_entitled_to_exemptions_under_151 : (List Individual) := ((fun (individual_list : (List Individual)) => ((fun (spouse_list : (List Individual)) => ((individual_list ++ spouse_list) ++ dependents)) (match spouse with
  | Optional.Absent _ => []
  | Optional.Present s => (if spouse_personal_exemption_allowed then [s] else [])))) [individual])

def IndividualSection151ExemptionsList_main_output_leaf_0 (input : IndividualSection151ExemptionsList_Input) : D IndividualSection151ExemptionsListOutput :=
  .ok (some (({ individual := input.individual, spouse_personal_exemption_allowed := input.spouse_personal_exemption_allowed, individuals_entitled_to_exemptions_under_151 := input.individuals_entitled_to_exemptions_under_151 } : IndividualSection151ExemptionsListOutput)))
...
def individualSection151ExemptionsList (input : IndividualSection151ExemptionsList_Input) : IndividualSection151ExemptionsList :=
  let main_output := match IndividualSection151ExemptionsList_main_output_leaf_0 input with | .ok (some val) => val | _ => { individual := input.individual, spouse_personal_exemption_allowed := input.spouse_personal_exemption_allowed, individuals_entitled_to_exemptions_under_151 := input.individuals_entitled_to_exemptions_under_151 }
...
structure IndividualSection152QualifyingChild_Input where
  ...
  age_requirement_met : Bool := ((fun (year_end : CatalaRuntime.Date) => ((fun (individual_birth_date : CatalaRuntime.Date) => ((fun (taxpayer_birth_date : CatalaRuntime.Date) => (decide ((individual_birth_date > taxpayer_birth_date)) && decide (((Date_en.is_old_enough year_end individual_birth_date (CatalaRuntime.Duration.create 25 0 0)) = false)))) (if (List.foldl ((fun (acc : Bool) (birth_event : BirthEvent) => (decide (acc) || decide (((birth_event).individual = taxpayer))))) false birth_events) then (match (List.map ((fun (birth_event : BirthEvent) => (birth_event).birth_date)) (List.filter ((fun (birth_event : BirthEvent) => ((birth_event).individual = taxpayer))) birth_events)) with | [] => (fun () => CatalaRuntime.Date.mk 0 0 0) () | x0 :: xn => List.foldl (fun (min1 : CatalaRuntime.Date) (min2 : CatalaRuntime.Date) => (if (min1 < min2) then min1 else min2)) x0 xn) else (Date_en.of_year_month_day (1900 : Int) (1 : Int) (1 : Int))))) (if (List.foldl ((fun (acc : Bool) (birth_event : BirthEvent) => (decide (acc) || decide (((birth_event).individual = individual))))) false birth_events) then (match (List.map ((fun (birth_event : BirthEvent) => (birth_event).birth_date)) (List.filter ((fun (birth_event : BirthEvent) => ((birth_event).individual = individual))) birth_events)) with | [] => (fun () => CatalaRuntime.Date.mk 0 0 0) () | x0 :: xn => List.foldl (fun (min1 : CatalaRuntime.Date) (min2 : CatalaRuntime.Date) => (if (min1 < min2) then min1 else min2)) x0 xn) else (Date_en.of_year_month_day (1900 : Int) (1 : Int) (1 : Int))))) (Date_en.of_year_month_day tax_year (12 : Int) (31 : Int)))
...
def individualSection152QualifyingChild (input : IndividualSection152QualifyingChild_Input) : IndividualSection152QualifyingChild :=
  let main_output := match IndividualSection152QualifyingChild_main_output_leaf_0 input with | .ok (some val) => val | _ => { individual := input.individual, taxpayer := input.taxpayer, is_qualifying_child := input.is_qualifying_child, relationship_requirement_met := input.relationship_requirement_met, principal_place_of_abode_requirement_met := input.principal_place_of_abode_requirement_met, age_requirement_met := input.age_requirement_met, joint_return_exception_applies := input.joint_return_exception_applies }
  { main_output := main_output }
...
def IndividualSection152QualifyingRelative_main_output_leaf_0 (input : IndividualSection152QualifyingRelative_Input) : D IndividualSection152QualifyingRelativeOutput :=
...
def individualSection152QualifyingRelative (input : IndividualSection152QualifyingRelative_Input) : IndividualSection152QualifyingRelative :=
  let main_output := match IndividualSection152QualifyingRelative_main_output_leaf_0 input with | .ok (some val) => val | _ => { individual := input.individual, taxpayer := input.taxpayer, is_qualifying_relative := input.is_qualifying_relative, relationship_requirement_met_H := input.relationship_requirement_met_H, relationship_requirement_met := input.relationship_requirement_met, no_income_requirement_met := input.no_income_requirement_met, not_qualifying_child_requirement_met := input.not_qualifying_child_requirement_met }
  { main_output := main_output }
...
structure IndividualSection7703MaritalStatus_Input where
  ...
  tax_year : Int
  individual : Individual
  section_7703_find_spouse_from_marriage_events : (Individual → (List MarriageEvent) → CatalaRuntime.Date → (Optional Individual)) := fun (individual_arg : Individual) (marriage_events_arg : (List MarriageEvent)) (year_end_arg : CatalaRuntime.Date) => ((fun (valid_marriages : (List (Individual × CatalaRuntime.Date))) => (if ((valid_marriages).length > (0 : Int)) then ((fun (most_recent_marriage : CatalaRuntime.Date) => (List_en.first_element (List.map ((fun (marriage_tuple : (Individual × CatalaRuntime.Date)) => (marriage_tuple).1)) (List.filter ((fun (marriage_tuple : (Individual × CatalaRuntime.Date)) => ((marriage_tuple).2 = most_recent_marriage))) valid_marriages)))) (match (List.map ((fun (marriage_tuple : (Individual × CatalaRuntime.Date)) => (marriage_tuple).2)) valid_marriages) with | [] => (fun () => CatalaRuntime.Date.mk 0 0 0 ) () | x0 :: xn => List.foldl (fun (max1 : CatalaRuntime.Date) (max2 : CatalaRuntime.Date) => (if (max1 > max2) then max1 else max2)) x0 xn)) else (Optional.Absent ()))) (List.map ((fun (marriage_event : MarriageEvent) => ((if ((marriage_event).spouse1 = individual_arg) then (marriage_event).spouse2 else (marriage_event).spouse1), (marriage_event).marriage_date))) (List.filter ((fun (marriage_event : MarriageEvent) => ((decide (((marriage_event).spouse1 = individual_arg)) || decide (((marriage_event).spouse2 = individual_arg))) && decide (((marriage_event).marriage_date ≤ year_end_arg))))) marriage_events_arg)))
  section_7703_get_spouse_death_date_during_year : ((Optional Individual) → (List DeathEvent) → Int → (Optional CatalaRuntime.Date)) := fun (spouse_arg : (Optional Individual)) (death_events_arg : (List DeathEvent)) (tax_year_arg : Int) => (match spouse_arg with
  | Optional.Absent _ => (Optional.Absent ())
  | Optional.Present s => ((fun (year_start : CatalaRuntime.Date) => ((fun (year_end : CatalaRuntime.Date) => ((fun (spouse_death_events_during_year : (List DeathEvent)) => (if ((spouse_death_events_during_year).length > (0 : Int)) then (Optional.Present (match (List.map ((fun (death_event : DeathEvent) => (death_event).death_date)) spouse_death_events_during_year) with | [] => (fun () => CatalaRuntime.Date.mk 0 0 0) () | x0 :: xn => List.foldl (fun (min1 : CatalaRuntime.Date) (min2 : CatalaRuntime.Date) => (if (min1 < min2) then min1 else min2)) x0 xn)) else (Optional.Absent ()))) (List.filter ((fun (death_event : DeathEvent) => (decide (((death_event).decedent = s)) && (decide (((death_event).death_date ≥ year_start)) && decide (((death_event).death_date ≤ year_end)))))) death_events_arg))) (Date_en.of_year_month_day tax_year_arg (12 : Int) (31 : Int)))) (Date_en.of_year_month_day tax_year_arg (1 : Int) (1 : Int))))
  section_7703_spouse_died_before_date : ((Optional Individual) → (List DeathEvent) → CatalaRuntime.Date → Bool) := fun (spouse_arg : (Optional Individual)) (death_events_arg : (List DeathEvent)) (before_date_arg : CatalaRuntime.Date) => (match spouse_arg with
  | Optional.Absent _ => false
  | Optional.Present s => ((fun (spouse_death_events_before_date : (List DeathEvent)) => ((spouse_death_events_before_date).length > (0 : Int))) (List.filter ((fun (death_event : DeathEvent) => (decide (((death_event).decedent = s)) && decide (((death_event).death_date < before_date_arg))))) death_events_arg)))
  section_7703_get_spouse_from_tax_return : (Individual → IndividualTaxReturn → (Optional Individual)) := fun (individual_arg : Individual) (individual_tax_return_arg : IndividualTaxReturn) => (match (individual_tax_return_arg).details with
  | FilingStatusVariant.JointReturn variant => (if ((variant).taxpayer = individual_arg) then (Optional.Present (variant).spouse) else (Optional.Absent ()))
  | FilingStatusVariant.SurvivingSpouse variant => (if ((variant).taxpayer = individual_arg) then (Optional.Present (variant).deceased_spouse) else (Optional.Absent ()))
  | FilingStatusVariant.HeadOfHousehold variant => (Optional.Absent ())
  | FilingStatusVariant.Single variant => (Optional.Absent ())
  | FilingStatusVariant.MarriedFilingSeparate variant => (if ((variant).taxpayer = individual_arg) then (Optional.Present (variant).spouse) else (Optional.Absent ())))
  section_7703_files_separate_return : (Individual → IndividualTaxReturn → Bool) := fun (individual_arg : Individual) (individual_tax_return_arg : IndividualTaxReturn) => (match (individual_tax_return_arg).details with
  | FilingStatusVariant.JointReturn variant => false
  | FilingStatusVariant.SurvivingSpouse variant => false
  | FilingStatusVariant.HeadOfHousehold variant => false
  | FilingStatusVariant.Single variant => false
  | FilingStatusVariant.MarriedFilingSeparate variant => ((variant).taxpayer = individual_arg))
  ...
  determination_date : CatalaRuntime.Date := ((fun (year_end : CatalaRuntime.Date) => ((fun (spouse : (Optional Individual)) => ((fun (spouse_death_date_during_year : (Optional CatalaRuntime.Date)) => (match spouse_death_date_during_year with
  | Optional.Absent _ => year_end
  | Optional.Present death_date => death_date)) (section_7703_get_spouse_death_date_during_year spouse death_events tax_year))) (section_7703_find_spouse_from_marriage_events individual marriage_events year_end))) (Date_en.of_year_month_day tax_year (12 : Int) (31 : Int)))
  ...
  is_legally_separated : Bool := ((fun (year_end : CatalaRuntime.Date) => ((fun (spouse : (Optional Individual)) => (match spouse with
  | Optional.Absent _ => false
  ...
  is_married_at_determination_date : Bool := ((fun (year_end : CatalaRuntime.Date) => ((fun (spouse : (Optional Individual)) => (match spouse with
  | Optional.Absent _ => false
  | Optional.Present s => (!decide ((section_7703_spouse_died_before_date spouse death_events determination_date))))) (section_7703_find_spouse_from_marriage_events individual marriage_events year_end))) (Date_en.of_year_month_day tax_year (12 : Int) (31 : Int)))
  spouse_not_member_of_household_last_6_months : Bool := ((fun (year_end : CatalaRuntime.Date) => ((fun (last_6_months_start : CatalaRuntime.Date) => ((fun (last_6_months_end : CatalaRuntime.Date) => ((fun (spouse_from_tax_return : (Optional Individual)) => (match spouse_from_tax_return with
  | Optional.Absent _ => ((households_maintained_by_individual).length > (0 : Int))
  | Optional.Present spouse => (List.foldl ((fun (acc : Bool) (household : Household) => (decide (acc) || (!decide ((section_7703_is_spouse_member_of_household_last_6_months spouse household residence_period_events last_6_months_start last_6_months_end)))))) false households_maintained_by_individual))) (section_7703_get_spouse_from_tax_return individual individual_tax_return))) year_end)) (Date_en.of_year_month_day tax_year (7 : Int) (1 : Int)))) (Date_en.of_year_month_day tax_year (12 : Int) (31 : Int)))
  subsection_b_exception_applies : Bool := (match processExceptions [if (decide (is_married_at_determination_date) && ((!decide (is_legally_separated)) && (decide ((section_7703_files_separate_return individual individual_tax_return)) && decide (spouse_not_member_of_household_last_6_months)))) then .ok (some (true)) else .ok none] with | .ok none => false | .ok (some r) => r | .error _ => false)
  is_married_for_tax_purposes : Bool := (match processExceptions [if (decide (is_married_at_determination_date) && (!decide (is_legally_separated))) then .ok (some (true)) else .ok none, if subsection_b_exception_applies then .ok (some (false)) else .ok none] with | .ok none => false | .ok (some r) => r | .error _ => false)

def IndividualSection7703MaritalStatus_main_output_leaf_0 (input : IndividualSection7703MaritalStatus_Input) : D IndividualSection7703MaritalStatusOutput :=
...
def individualSection7703MaritalStatus (input : IndividualSection7703MaritalStatus_Input) : IndividualSection7703MaritalStatus :=
  let main_output := match IndividualSection7703MaritalStatus_main_output_leaf_0 input with | .ok (some val) => val | _ => { individual := input.individual, tax_year := input.tax_year, determination_date := input.determination_date, is_married_at_determination_date := input.is_married_at_determination_date, is_legally_separated := input.is_legally_separated, households_with_qualifying_child := input.households_with_qualifying_child, households_maintained_by_individual := input.households_maintained_by_individual, spouse_not_member_of_household_last_6_months := input.spouse_not_member_of_household_last_6_months, subsection_b_exception_applies := input.subsection_b_exception_applies, is_married_for_tax_purposes := input.is_married_for_tax_purposes }
  { main_output := main_output }
...
structure OrganizationSection3306EmployerStatus_Input where
  ...
  is_agricultural_employer : Bool := (match processExceptions [if ((fun (relevant_employment_events : (List EmploymentRelationshipEvent)) => (section_3306_has_ten_days_in_different_weeks relevant_employment_events calendar_year (5 : Int))) (List.filter ((fun (emp_event : EmploymentRelationshipEvent) => (decide ((((emp_event).employer).id = (organization).id)) && (decide (((emp_event).employment_category = (EmploymentCategory.AgriculturalLabor ()))) && (decide (((Date_en.get_year (emp_event).start_date) = calendar_year)) || (decide (((Date_en.get_year (emp_event).start_date) = (calendar_year - (1 : Int)))) || (decide (((Date_en.get_year (emp_event).end_date) = calendar_year)) || decide (((Date_en.get_year (emp_event).end_date) = (calendar_year - (1 : Int))))))))))) employment_relationship_events)) then .ok (some (true)) else .ok none] with | .ok none => ((fun (relevant_wage_events : (List WagePaymentEvent)) => ((fun (total_agricultural_wages : CatalaRuntime.Money) => (total_agricultural_wages ≥ (CatalaRuntime.Money.ofCents 2000000))) (match (List.map ((fun (wage_event : WagePaymentEvent) => (wage_event).amount)) relevant_wage_events) with | [] => (fun () => (CatalaRuntime.Money.ofCents 0)) () | x0 :: xn => List.foldl (fun (sum1 : CatalaRuntime.Money) (sum2 : CatalaRuntime.Money) => (sum1 + sum2)) x0 xn))) (List.filter ((fun (wage_event : WagePaymentEvent) => (decide ((((wage_event).employer).id = (organization).id)) && ((decide (((Date_en.get_year (wage_event).payment_date) = calendar_year)) || decide (((Date_en.get_year (wage_event).payment_date) = (calendar_year - (1 : Int))))) && decide ((List.foldl ((fun (acc : Bool) (emp_event : EmploymentRelationshipEvent) => (decide (acc) || (decide ((((emp_event).employer).id = (organization).id)) && (decide ((((emp_event).employee).id = ((wage_event).employee).id)) && decide (((emp_event).employment_category = (EmploymentCategory.AgriculturalLabor ())))))))) false employment_relationship_events)))))) wage_payment_events)) | .ok (some r) => r | .error _ => false)
  is_general_employer : Bool := (match processExceptions [if ((fun (relevant_employment_events : (List EmploymentRelationshipEvent)) => (section_3306_has_ten_days_in_different_weeks relevant_employment_events calendar_year (1 : Int))) (List.filter ((fun (emp_event : EmploymentRelationshipEvent) => (decide ((((emp_event).employer).id = (organization).id)) && ((decide (((Date_en.get_year (emp_event).start_date) = calendar_year)) || (decide (((Date_en.get_year (emp_event).start_date) = (calendar_year - (1 : Int)))) || (decide (((Date_en.get_year (emp_event).end_date) = calendar_year)) || decide (((Date_en.get_year (emp_event).end_date) = (calendar_year - (1 : Int))))))) && (!decide (((emp_event).employment_category = (EmploymentCategory.DomesticService ())))))))) employment_relationship_events)) then .ok (some (true)) else .ok none] with | .ok none => ((fun (relevant_wage_events : (List WagePaymentEvent)) => ((fun (total_wages : CatalaRuntime.Money) => (total_wages ≥ (CatalaRuntime.Money.ofCents 150000))) (match (List.map ((fun (wage_event : WagePaymentEvent) => (wage_event).amount)) relevant_wage_events) with | [] => (fun () => (CatalaRuntime.Money.ofCents 0)) () | x0 :: xn => List.foldl (fun (sum1 : CatalaRuntime.Money) (sum2 : CatalaRuntime.Money) => (sum1 + sum2)) x0 xn))) (List.filter ((fun (wage_event : WagePaymentEvent) => (decide ((((wage_event).employer).id = (organization).id)) && ((decide (((Date_en.get_year (wage_event).payment_date) = calendar_year)) || decide (((Date_en.get_year (wage_event).payment_date) = (calendar_year - (1 : Int))))) && (!decide ((List.foldl ((fun (acc : Bool) (emp_event : EmploymentRelationshipEvent) => (decide (acc) || (decide ((((emp_event).employer).id = (organization).id)) && (decide ((((emp_event).employee).id = ((wage_event).employee).id)) && decide (((emp_event).employment_category = (EmploymentCategory.DomesticService ())))))))) false employment_relationship_events))))))) wage_payment_events)) | .ok (some r) => r | .error _ => false)
  is_employer : Bool := (decide (is_general_employer) || (decide (is_agricultural_employer) || decide (is_domestic_service_employer)))

def OrganizationSection3306EmployerStatus_main_output_leaf_0 (input : OrganizationSection3306EmployerStatus_Input) : D OrganizationSection3306EmployerStatusOutput :=
  .ok (some (({ organization := input.organization, is_employer := input.is_employer, is_general_employer := input.is_general_employer, is_agricultural_employer := input.is_agricultural_employer, is_domestic_service_employer := input.is_domestic_service_employer } : OrganizationSection3306EmployerStatusOutput)))
...
def organizationSection3306EmployerStatus (input : OrganizationSection3306EmployerStatus_Input) : OrganizationSection3306EmployerStatus :=
  let main_output := match OrganizationSection3306EmployerStatus_main_output_leaf_0 input with | .ok (some val) => val | _ => { organization := input.organization, is_employer := input.is_employer, is_general_employer := input.is_general_employer, is_agricultural_employer := input.is_agricultural_employer, is_domestic_service_employer := input.is_domestic_service_employer }
  { main_output := main_output }
...
def WagePaymentEventSection3306Wages_main_output_leaf_0 (input : WagePaymentEventSection3306Wages_Input) : D WagePaymentEventSection3306WagesOutput :=
  .ok (some (({ wage_payment_event := input.wage_payment_event, is_excluded_by_sickness_disability_death := input.is_excluded_by_sickness_disability_death, is_excluded_by_nonbusiness_service := input.is_excluded_by_nonbusiness_service, is_excluded_by_termination_payment := input.is_excluded_by_termination_payment, is_excluded_by_agricultural_noncash := input.is_excluded_by_agricultural_noncash, is_excluded_by_survivor_payment := input.is_excluded_by_survivor_payment, taxable_amount_before_7000_cap := input.taxable_amount_before_7000_cap } : WagePaymentEventSection3306WagesOutput)))
...
def wagePaymentEventSection3306Wages (input : WagePaymentEventSection3306Wages_Input) : WagePaymentEventSection3306Wages :=
  let main_output := match WagePaymentEventSection3306Wages_main_output_leaf_0 input with | .ok (some val) => val | _ => { wage_payment_event := input.wage_payment_event, is_excluded_by_sickness_disability_death := input.is_excluded_by_sickness_disability_death, is_excluded_by_nonbusiness_service := input.is_excluded_by_nonbusiness_service, is_excluded_by_termination_payment := input.is_excluded_by_termination_payment, is_excluded_by_agricultural_noncash := input.is_excluded_by_agricultural_noncash, is_excluded_by_survivor_payment := input.is_excluded_by_survivor_payment, taxable_amount_before_7000_cap := input.taxable_amount_before_7000_cap }
  { main_output := main_output }
...
def TotalWages3306Calculation_main_output_leaf_0 (input : TotalWages3306Calculation_Input) : D TotalWages3306CalculationOutput :=
  .ok (some (({ total_taxable_wages := input.total_taxable_wages, wage_results_with_cap := input.wage_results } : TotalWages3306CalculationOutput)))
...
def totalWages3306Calculation (input : TotalWages3306Calculation_Input) : TotalWages3306Calculation :=
  let main_output := match TotalWages3306Calculation_main_output_leaf_0 input with | .ok (some val) => val | _ => { total_taxable_wages := input.total_taxable_wages, wage_results_with_cap := input.wage_results }
  { main_output := main_output }
...
def IndividualSection152Dependents_main_output_leaf_0 (input : IndividualSection152Dependents_Input) : D IndividualSection152DependentsOutput :=
  .ok (some (({ taxpayer := input.taxpayer, dependents_initial := input.dependents_initial, dependents_after_152b1 := input.dependents_after_152b1, dependents_after_152b2 := input.dependents_after_152b2, qualifying_children := input.qualifying_children, qualifying_relatives := input.qualifying_relatives } : IndividualSection152DependentsOutput)))
...
def individualSection152Dependents (input : IndividualSection152Dependents_Input) : IndividualSection152Dependents :=
  let main_output := match IndividualSection152Dependents_main_output_leaf_0 input with | .ok (some val) => val | _ => { taxpayer := input.taxpayer, dependents_initial := input.dependents_initial, dependents_after_152b1 := input.dependents_after_152b1, dependents_after_152b2 := input.dependents_after_152b2, qualifying_children := input.qualifying_children, qualifying_relatives := input.qualifying_relatives }
  { main_output := main_output }
...
structure EmploymentRelationshipEventSection3306Employment_Input where
  ...
  is_excluded_domestic_service : Bool := (if ((employment_relationship_event).employment_category = (EmploymentCategory.DomesticService ())) then ((fun (employer_local : Organization) => ((fun (employer_status : OrganizationSection3306EmployerStatusOutput) => ((fun (employer_is_domestic_service_employer : Bool) => (!decide (employer_is_domestic_service_employer))) (employer_status).is_domestic_service_employer)) ((organizationSection3306EmployerStatus {employment_relationship_events:=employment_relationship_events,wage_payment_events:=wage_payment_events,calendar_year:=calendar_year,organization:=employer_local})).main_output)) (employment_relationship_event).employer) else false)
  ...
  is_excluded_agricultural_labor : Bool := (if ((employment_relationship_event).employment_category = (EmploymentCategory.AgriculturalLabor ())) then ((fun (employer_local : Organization) => ((fun (employer_status : OrganizationSection3306EmployerStatusOutput) => ((fun (employer_is_agricultural_employer : Bool) => ((fun (is_h2a_alien : Bool) => ((!decide (employer_is_agricultural_employer)) || decide (is_h2a_alien))) (List.foldl ((fun (acc : Bool) (iae : ImmigrationAdmissionEvent) => (decide (acc) || (decide ((((iae).individual).id = ((employment_relationship_event).employee).id)) && decide (((iae).visa_category = (VisaCategory.H2A ()))))))) false immigration_admission_events))) (employer_status).is_agricultural_employer)) ((organizationSection3306EmployerStatus {employment_relationship_events:=employment_relationship_events,wage_payment_events:=wage_payment_events,calendar_year:=calendar_year,organization:=employer_local})).main_output)) (employment_relationship_event).employer) else false)
...
def employmentRelationshipEventSection3306Employment (input : EmploymentRelationshipEventSection3306Employment_Input) : EmploymentRelationshipEventSection3306Employment :=
  let main_output := match EmploymentRelationshipEventSection3306Employment_main_output_leaf_0 input with | .ok (some val) => val | _ => { employment_relationship_event := input.employment_relationship_event, is_employment := input.is_employment, is_excluded_agricultural_labor := input.is_excluded_agricultural_labor, is_excluded_domestic_service := input.is_excluded_domestic_service, is_excluded_family_employment := input.is_excluded_family_employment, is_excluded_federal_government := input.is_excluded_federal_government, is_excluded_state_government := input.is_excluded_state_government, is_excluded_student_service := input.is_excluded_student_service, is_excluded_hospital_patient_service := input.is_excluded_hospital_patient_service, is_excluded_foreign_government := input.is_excluded_foreign_government, is_excluded_student_nurse := input.is_excluded_student_nurse, is_excluded_international_organization := input.is_excluded_international_organization, is_excluded_penal_institution := input.is_excluded_penal_institution }
  { main_output := main_output }
...
def employerUnemploymentExciseTaxFilerSection3301Tax (input : EmployerUnemploymentExciseTaxFilerSection3301Tax_Input) : EmployerUnemploymentExciseTaxFilerSection3301Tax :=
  let main_output := match EmployerUnemploymentExciseTaxFilerSection3301Tax_main_output_leaf_0 input with | .ok (some val) => val | _ => { employer_unemployment_excise_tax_return := input.employer_unemployment_excise_tax_return, total_taxable_wages := input.total_taxable_wages, excise_tax := input.excise_tax, tax_rate := input.tax_rate }
...
structure IndividualSection151Exemption_Input where
  ...
  applicable_percentage : Rat := (if (adjusted_gross_income > applicable_amount) then ((fun (excess_agi : CatalaRuntime.Money) => ((fun (threshold : CatalaRuntime.Money) => ((fun (excess_decimal : Rat) => ((fun (threshold_decimal : Rat) => ((fun (fraction_count_decimal : Rat) => ((fun (fraction_count : Rat) => (Decimal_en.min (CatalaRuntime.multiply fraction_count (Rat.mk 1 50)) (Rat.mk 1 1))) (if (fraction_count_decimal > (CatalaRuntime.toRat (Rat.floor fraction_count_decimal))) then (CatalaRuntime.toRat ((Rat.floor fraction_count_decimal) + (1 : Int))) else (CatalaRuntime.toRat (Rat.floor fraction_count_decimal))))) (excess_decimal / threshold_decimal))) (CatalaRuntime.toRat threshold))) (CatalaRuntime.toRat excess_agi))) (match (individual_tax_return).details with
  | FilingStatusVariant.JointReturn _ => (CatalaRuntime.Money.ofCents 250000)
  | FilingStatusVariant.SurvivingSpouse _ => (CatalaRuntime.Money.ofCents 250000)
  | FilingStatusVariant.HeadOfHousehold _ => (CatalaRuntime.Money.ofCents 250000)
  | FilingStatusVariant.Single _ => (CatalaRuntime.Money.ofCents 250000)
  | FilingStatusVariant.MarriedFilingSeparate variant => (CatalaRuntime.Money.ofCents 125000)))) (adjusted_gross_income - applicable_amount)) else (Rat.mk 0 1))
  ...
  exemption_amount_after_phaseout : CatalaRuntime.Money := (match processExceptions [if (is_tax_year_2018_through_2025 tax_year) then .ok (some ((CatalaRuntime.Money.ofCents 0))) else .ok none] with | .ok none => (if (adjusted_gross_income > applicable_amount) then ((fun (reduction : CatalaRuntime.Money) => (exemption_amount_after_disallowance - reduction)) (CatalaRuntime.toMoney (CatalaRuntime.multiply (CatalaRuntime.toRat exemption_amount_after_disallowance) applicable_percentage))) else exemption_amount_after_disallowance) | .ok (some r) => r | .error _ => CatalaRuntime.Money.ofCents (-100))
...
def IndividualSection151Exemption_main_output_leaf_0 (input : IndividualSection151Exemption_Input) : D IndividualSection151ExemptionOutput :=
...
def individualSection151Exemption (input : IndividualSection151Exemption_Input) : IndividualSection151Exemption :=
  let main_output := match IndividualSection151Exemption_main_output_leaf_0 input with | .ok (some val) => val | _ => { individual := input.individual, exemption_amount_base := input.exemption_amount_base, exemption_amount_after_disallowance := input.exemption_amount_after_disallowance, exemption_amount_after_phaseout := input.exemption_amount_after_phaseout, number_of_personal_exemptions := input.number_of_personal_exemptions, personal_exemptions_deduction := input.personal_exemptions_deduction, applicable_percentage := input.applicable_percentage }
  { main_output := main_output }
...
structure TaxpayerExemptionsList_Input where
  ...
  tax_return_events : (List TaxReturnEvent)
  individual_tax_return : IndividualTaxReturn
  spouse_result : (Optional IndividualSection151ExemptionsListOutput) := (match (individual_tax_return).details with
  | FilingStatusVariant.JointReturn variant => ((fun (spouse_local : (Optional Individual)) => ((fun (spouse_result_inner : (Optional IndividualSection151ExemptionsListOutput)) => spouse_result_inner) (match spouse_local with
  | Optional.Absent _ => (Optional.Absent ())
  | Optional.Present s => ((fun (taxpayer_local : Individual) => ((fun (spouse_result_scope : IndividualSection151ExemptionsList) => (Optional.Present (spouse_result_scope).main_output)) (individualSection151ExemptionsList {is_joint_or_surviving_spouse:=true,dependents:=dependents,income_events:=income_events,tax_return_events:=tax_return_events,tax_year:=(individual_tax_return).tax_year,spouse:=(Optional.Present taxpayer_local),individual:=s}))) (get_taxpayer individual_tax_return))))) (get_spouse individual_tax_return))
  | FilingStatusVariant.SurvivingSpouse variant => (Optional.Absent ())
  | FilingStatusVariant.HeadOfHousehold variant => (Optional.Absent ())
  | FilingStatusVariant.Single variant => (Optional.Absent ())
  | FilingStatusVariant.MarriedFilingSeparate variant => (Optional.Absent ()))
  ...
  | FilingStatusVariant.JointReturn variant => true
  | FilingStatusVariant.SurvivingSpouse variant => true
  | FilingStatusVariant.HeadOfHousehold variant => false
  | FilingStatusVariant.Single variant => false
  | FilingStatusVariant.MarriedFilingSeparate variant => false) }
  spouse_personal_exemption_allowed : Bool := ((taxpayer_exemptions).main_output).spouse_personal_exemption_allowed
  individuals_entitled_to_exemptions_under_151 : (List Individual) := ((fun (taxpayer_list : (List Individual)) => ((fun (spouse_unique_list : (List Individual)) => (taxpayer_list ++ spouse_unique_list)) (match spouse_result with
  | Optional.Absent _ => []
  | Optional.Present s_result => (List.filter ((fun (individual : Individual) => (!decide ((List.foldl ((fun (acc : Bool) (taxpayer_individual : Individual) => (decide (acc) || decide ((taxpayer_individual = individual))))) false taxpayer_list))))) (s_result).individuals_entitled_to_exemptions_under_151)))) ((taxpayer_exemptions).main_output).individuals_entitled_to_exemptions_under_151)

def TaxpayerExemptionsList_main_output_leaf_0 (input : TaxpayerExemptionsList_Input) (taxpayer_exemptions : IndividualSection151ExemptionsList) : D TaxpayerExemptionsListOutput :=
  .ok (some (({ taxpayer_result := (taxpayer_exemptions).main_output, spouse_result := input.spouse_result, individuals_entitled_to_exemptions_under_151 := input.individuals_entitled_to_exemptions_under_151, spouse_personal_exemption_allowed := input.spouse_personal_exemption_allowed } : TaxpayerExemptionsListOutput)))
...
def taxpayerExemptionsList (input : TaxpayerExemptionsList_Input) : TaxpayerExemptionsList :=
  ...
  | FilingStatusVariant.Single variant => false
  | FilingStatusVariant.MarriedFilingSeparate variant => false) }
  let main_output := match TaxpayerExemptionsList_main_output_leaf_0 input taxpayer_exemptions with | .ok (some val) => val | _ => { taxpayer_result := (taxpayer_exemptions).main_output, spouse_result := input.spouse_result, individuals_entitled_to_exemptions_under_151 := input.individuals_entitled_to_exemptions_under_151, spouse_personal_exemption_allowed := input.spouse_personal_exemption_allowed }
  { main_output := main_output }
...
structure IRCSimplified_Input where
  ...
  adjusted_gross_income : CatalaRuntime.Money
  section_2_a_1_A_spouse_died_in_preceding_two_years : Bool := ((fun (spouse_local : (Optional Individual)) => (match spouse_local with
  | Optional.Absent _ => false
  ...
  section_2_a_2_B_joint_return_could_have_been_made : Bool := ((fun (spouse_local : (Optional Individual)) => ((fun (taxpayer_local : Individual) => ((fun (spouse_death_events_in_window_local : (List DeathEvent)) => ((fun (spouse_death_year_local : (Optional Int)) => (match spouse_death_year_local with
  | Optional.Absent _ => false
  | Optional.Present death_year => ((!decide ((List.foldl ((fun (acc : Bool) (residency_event_taxpayer : NonresidentAlienStatusPeriodEvent) => (decide (acc) || (decide (((residency_event_taxpayer).individual = taxpayer_local)) && (decide (((residency_event_taxpayer).residency_status = (ResidencyStatus.NonresidentAlien ()))) && (decide (((residency_event_taxpayer).start_date ≤ (Date_en.of_year_month_day death_year (12 : Int) (31 : Int)))) && decide (((residency_event_taxpayer).end_date ≥ (Date_en.of_year_month_day death_year (1 : Int) (1 : Int)))))))))) false nonresident_alien_status_period_events))) && (!((match spouse_local with
  | Optional.Absent _ => false
  ...
  | FilingStatusVariant.JointReturn variant => false
  | FilingStatusVariant.SurvivingSpouse variant => false
  | FilingStatusVariant.HeadOfHousehold variant => ((fun (qualifying_person_is_qualifying_child_local : Bool) => (decide (qualifying_person_is_qualifying_child_local) && (!(decide (section_2_b_1_A_i_I_qualifying_child_is_married_at_close_of_year) && decide (section_2_b_1_A_i_II_qualifying_person_not_dependent_by_152b2))))) ((fun (result : IndividualSection152QualifyingChild) => ((result).main_output).is_qualifying_child) (individualSection152QualifyingChild {tax_return_events:=tax_return_events,residence_period_events:=residence_period_events,birth_events:=birth_events,family_relationship_events:=family_relationship_events,tax_year:=(individual_tax_return).tax_year,taxpayer:=(get_taxpayer individual_tax_return),individual:=(variant).qualifying_person})))
  | FilingStatusVariant.Single variant => false
  | FilingStatusVariant.MarriedFilingSeparate variant => false)
  taxpayer_marital_status : IndividualSection7703MaritalStatus :=
  individualSection7703MaritalStatus {
    individual := (match (individual_tax_return).details with
      | FilingStatusVariant.JointReturn variant => (variant).taxpayer
      | FilingStatusVariant.SurvivingSpouse variant => (variant).taxpayer
      | FilingStatusVariant.HeadOfHousehold variant => (variant).taxpayer
      | FilingStatusVariant.Single variant => (variant).taxpayer
    ...
    divorce_or_legal_separation_events := divorce_or_legal_separation_events,
    death_events := death_events,
    individual_tax_return := individual_tax_return,
    residence_period_events := residence_period_events,
    household_maintenance_events := household_maintenance_events,
    qualifying_children := (taxpayer_dependents_result).qualifying_children,
    individuals_entitled_to_exemptions_under_151 := (taxpayer_exemptions_list_result).individuals_entitled_to_exemptions_under_151 }
  additional_amount_spouse_blind : CatalaRuntime.Money :=
   (match processExceptions [if ((fun (spouse_local : (Optional Individual)) =>
    ((fun (year_end_local : CatalaRuntime.Date) =>
      ((fun (spouse_death_events_local : (List DeathEvent)) =>
        ((fun (spouse_death_date_local : CatalaRuntime.Date) =>
          ((fun (spouse_is_blind_at_close_local : Bool) =>
            (decide (spouse_is_blind_at_close_local) &&
            (decide ((taxpayer_exemptions_list_result).spouse_personal_exemption_allowed) &&
             ((match (individual_tax_return).details with
              | FilingStatusVariant.JointReturn variant => true
              | FilingStatusVariant.SurvivingSpouse variant => true
              | FilingStatusVariant.HeadOfHousehold variant => false
              | FilingStatusVariant.Single variant => false
              | FilingStatusVariant.MarriedFilingSeparate variant => true))))) (match spouse_local with
  | Optional.Absent _ => false
  | Optional.Present s => (individual_is_blind_at_close s blindness_status_events death_events spouse_death_date_local)))) (if ((spouse_death_events_local).length > (0 : Int)) then (match (List.map ((fun (death_event : DeathEvent) => (death_event).death_date)) spouse_death_events_local) with | [] => (fun () => CatalaRuntime.Date.mk 0 0 0) () | x0 :: xn => List.foldl (fun (min1 : CatalaRuntime.Date) (min2 : CatalaRuntime.Date) => (if (min1 < min2) then min1 else min2)) x0 xn) else year_end_local))) (match spouse_local with
  | Optional.Absent _ => (List.filter ((fun (death_event : DeathEvent) => false)) death_events)
  | Optional.Present s => (List.filter ((fun (death_event : DeathEvent) => (decide (((death_event).decedent = s)) && decide (((Date_en.get_year (death_event).death_date) = (individual_tax_return).tax_year))))) death_events)))) (get_year_end (individual_tax_return).tax_year))) (get_spouse individual_tax_return)) then .ok (some ((CatalaRuntime.Money.ofCents 60000))) else .ok none] with | .ok none => (CatalaRuntime.Money.ofCents 0) | .ok (some r) => r | .error _ => (CatalaRuntime.Money.ofCents (-100)))
  additional_amount_spouse_aged : CatalaRuntime.Money := (match processExceptions [if ((fun (spouse_local : (Optional Individual)) => ((fun (year_end_local : CatalaRuntime.Date) => ((fun (spouse_attained_age_65_local : Bool) => (decide (spouse_attained_age_65_local) && (decide ((taxpayer_exemptions_list_result).spouse_personal_exemption_allowed) &&
   ((match (individual_tax_return).details with
  | FilingStatusVariant.JointReturn variant => true
  | FilingStatusVariant.SurvivingSpouse variant => true
  | FilingStatusVariant.HeadOfHousehold variant => false
  ...
  | FilingStatusVariant.MarriedFilingSeparate variant => (variant).itemization_election)) || decide ((extract_spouse_itemization_election individual_tax_return))))) then .ok (some (false)) else .ok none] with | .ok none => true | .ok (some r) => r | .error _ => false)
  ...
  | FilingStatusVariant.JointReturn variant => false
  | FilingStatusVariant.SurvivingSpouse variant => false
  | FilingStatusVariant.HeadOfHousehold variant => (List.foldl ((fun (acc : Bool) (maintenance_event : HouseholdMaintenanceEvent) => (decide (acc) || (decide (((maintenance_event).individual = taxpayer_local)) && (decide (((maintenance_event).start_date ≤ year_end_local)) && (decide (((maintenance_event).end_date ≥ (Date_en.of_year_month_day (individual_tax_return).tax_year (1 : Int) (1 : Int)))) && (decide (((maintenance_event).cost_furnished_percentage > (Rat.mk 1 2))) && decide ((List.foldl ((fun (acc : Bool) (residence_event : ResidencePeriodEvent) => (decide (acc) || (decide (((residence_event).individual = (variant).qualifying_person)) && (decide (((residence_event).household = (maintenance_event).household)) && (decide ((residence_event).is_member_of_household) && (decide ((residence_event).is_principal_place_of_abode) && (decide (((residence_event).start_date ≤ (Date_en.of_year_month_day (individual_tax_return).tax_year (6 : Int) (30 : Int)))) && decide (((residence_event).end_date ≥ (Date_en.of_year_month_day (individual_tax_return).tax_year (7 : Int) (1 : Int)))))))))))) false residence_period_events))))))))) false household_maintenance_events)
  | FilingStatusVariant.Single variant => false
  | FilingStatusVariant.MarriedFilingSeparate variant => false))) (get_year_end (individual_tax_return).tax_year))) (get_taxpayer individual_tax_return))
  is_surviving_spouse : Bool := (match processExceptions [if section_2_a_2_A_taxpayer_has_remarried then .ok (some (false)) else .ok none, if (!decide (section_2_a_2_B_joint_return_could_have_been_made)) then .ok (some (false)) else .ok none] with | .ok none => (decide (section_2_a_1_A_spouse_died_in_preceding_two_years) && decide (section_2_a_1_B_satisfied)) | .ok (some r) => r | .error _ => false)
  is_head_of_household : Bool := (match processExceptions [if ((fun (taxpayer_local : Individual) => ((fun (year_end_local : CatalaRuntime.Date) => (List.foldl ((fun (acc : Bool) (residency_event : NonresidentAlienStatusPeriodEvent) => (decide (acc) || (decide (((residency_event).individual = taxpayer_local)) && (decide (((residency_event).start_date ≤ year_end_local)) && (decide (((residency_event).end_date ≥ (Date_en.of_year_month_day (individual_tax_return).tax_year (1 : Int) (1 : Int)))) && decide (((residency_event).residency_status = (ResidencyStatus.NonresidentAlien ()))))))))) false nonresident_alien_status_period_events)) (get_year_end (individual_tax_return).tax_year))) (get_taxpayer individual_tax_return)) then .ok (some (false)) else .ok none, if (match (individual_tax_return).details with
  | FilingStatusVariant.JointReturn variant => false
  | FilingStatusVariant.SurvivingSpouse variant => false
  | FilingStatusVariant.HeadOfHousehold variant => (List.foldl ((fun (acc : Bool) (result : IndividualSection152QualifyingRelativeOutput) => (decide (acc) || (decide (((result).individual = (variant).qualifying_person)) && (decide (((result).taxpayer = (get_taxpayer individual_tax_return))) && (decide ((result).is_qualifying_relative) && ((!decide ((result).relationship_requirement_met)) && decide ((result).relationship_requirement_met_H)))))))) false (taxpayer_dependents_result).qualifying_relatives)
  | FilingStatusVariant.Single variant => false
  | FilingStatusVariant.MarriedFilingSeparate variant => false) then .ok (some (false)) else .ok none] with | .ok none => ((!decide (section_2_b_2_is_married_at_close_of_year)) && ((!decide (is_surviving_spouse)) && (decide (section_2_b_1_A_satisfied) || decide (section_2_b_1_B_satisfied)))) | .ok (some r) => r | .error _ => false)
  additional_amount_taxpayer_blind : CatalaRuntime.Money := (match processExceptions [if ((fun (taxpayer_local : Individual) => ((fun (year_end_local : CatalaRuntime.Date) => (decide ((individual_is_blind_at_close taxpayer_local blindness_status_events death_events year_end_local)) && ((!decide (((taxpayer_marital_status).main_output).is_married_for_tax_purposes)) && (!decide (is_surviving_spouse))))) (get_year_end (individual_tax_return).tax_year))) (get_taxpayer individual_tax_return)) then .ok (some ((CatalaRuntime.Money.ofCents 75000))) else .ok none] with | .ok none => (if (individual_is_blind_at_close (get_taxpayer individual_tax_return) blindness_status_events death_events (get_year_end (individual_tax_return).tax_year)) then (CatalaRuntime.Money.ofCents 60000) else (CatalaRuntime.Money.ofCents 0)) | .ok (some r) => r | .error _ => CatalaRuntime.Money.ofCents (-100))
  additional_amount_taxpayer_aged : CatalaRuntime.Money := (match processExceptions [if ((fun (taxpayer_local : Individual) => ((fun (year_end_local : CatalaRuntime.Date) => (decide ((individual_attained_age_65 taxpayer_local birth_events year_end_local)) && ((!decide (((taxpayer_marital_status).main_output).is_married_for_tax_purposes)) && (!decide (is_surviving_spouse))))) (get_year_end (individual_tax_return).tax_year))) (get_taxpayer individual_tax_return)) then .ok (some ((CatalaRuntime.Money.ofCents 75000))) else .ok none] with | .ok none => (if (individual_attained_age_65 (get_taxpayer individual_tax_return) birth_events (get_year_end (individual_tax_return).tax_year)) then (CatalaRuntime.Money.ofCents 60000) else (CatalaRuntime.Money.ofCents 0)) | .ok (some r) => r | .error _ => CatalaRuntime.Money.ofCents (-100))
  applicable_amount : CatalaRuntime.Money := (match processExceptions [if is_head_of_household then .ok (some ((CatalaRuntime.Money.ofCents 27500000))) else .ok none, if ((!decide (((taxpayer_marital_status).main_output).is_married_for_tax_purposes)) && ((!decide (is_surviving_spouse)) && (!decide (is_head_of_household)))) then .ok (some ((CatalaRuntime.Money.ofCents 25000000))) else .ok none, if (decide (((taxpayer_marital_status).main_output).is_married_for_tax_purposes) &&  ((match (individual_tax_return).details with
  | FilingStatusVariant.JointReturn variant => false
  | FilingStatusVariant.SurvivingSpouse variant => false
  | FilingStatusVariant.HeadOfHousehold variant => false
  | FilingStatusVariant.Single variant => false
  | FilingStatusVariant.MarriedFilingSeparate variant => true))) then .ok (some ((CatalaRuntime.Money.ofCents 15000000))) else .ok none] with | .ok none => (if ((decide (((taxpayer_marital_status).main_output).is_married_for_tax_purposes) && ((match (individual_tax_return).details with
  | FilingStatusVariant.JointReturn variant => true
  | FilingStatusVariant.SurvivingSpouse variant => false
  | FilingStatusVariant.HeadOfHousehold variant => false
  | FilingStatusVariant.Single variant => false
  | FilingStatusVariant.MarriedFilingSeparate variant => false))) || decide (is_surviving_spouse)) then (CatalaRuntime.Money.ofCents 30000000) else (CatalaRuntime.Money.ofCents 0)) | .ok (some r) => r | .error _ => CatalaRuntime.Money.ofCents (-100))
  basic_standard_deduction : CatalaRuntime.Money := (match processExceptions [if ((!decide ((is_tax_year_2018_through_2025 (individual_tax_return).tax_year))) && decide (is_head_of_household)) then .ok (some ((CatalaRuntime.Money.ofCents 440000))) else .ok none, if ((!decide ((is_tax_year_2018_through_2025 (individual_tax_return).tax_year))) && ((decide (((taxpayer_marital_status).main_output).is_married_for_tax_purposes) && ((match (individual_tax_return).details with
  | FilingStatusVariant.JointReturn variant => true
  | FilingStatusVariant.SurvivingSpouse variant => false
  | FilingStatusVariant.HeadOfHousehold variant => false
  | FilingStatusVariant.Single variant => false
  | FilingStatusVariant.MarriedFilingSeparate variant => false))) || decide (is_surviving_spouse))) then .ok (some ((CatalaRuntime.Money.ofCents 600000))) else .ok none, if (is_tax_year_2018_through_2025 (individual_tax_return).tax_year) then .ok (some ((CatalaRuntime.Money.ofCents 1200000))) else .ok none, if (decide ((is_tax_year_2018_through_2025 (individual_tax_return).tax_year)) && decide (is_head_of_household)) then .ok (some ((CatalaRuntime.Money.ofCents 1800000))) else .ok none, if (decide ((is_tax_year_2018_through_2025 (individual_tax_return).tax_year)) && ((decide (((taxpayer_marital_status).main_output).is_married_for_tax_purposes) && ((match (individual_tax_return).details with
  | FilingStatusVariant.JointReturn variant => true
  | FilingStatusVariant.SurvivingSpouse variant => false
  | FilingStatusVariant.HeadOfHousehold variant => false
  | FilingStatusVariant.Single variant => false
  | FilingStatusVariant.MarriedFilingSeparate variant => false))) || decide (is_surviving_spouse))) then .ok (some ((CatalaRuntime.Money.ofCents 2400000))) else .ok none, if ((fun (taxpayer_local : Individual) => ((fun (deduction_allowable_to_another_taxpayer : Bool) => deduction_allowable_to_another_taxpayer) (List.foldl ((fun (acc : Bool) (event : TaxReturnEvent) => (decide (acc) || ((!decide (((event).individual = taxpayer_local))) && (decide (((event).tax_year = (individual_tax_return).tax_year)) && decide ((List.foldl ((fun (acc : Bool) (dependent : Individual) => (decide (acc) || decide ((dependent = taxpayer_local))))) false (event).dependents))))))) false tax_return_events))) (get_taxpayer individual_tax_return)) then .ok (some (((fun (taxpayer_local : Individual) => ((fun (earned_income_local : CatalaRuntime.Money) => ((Money_en.min (CatalaRuntime.Money.ofCents 50000) (CatalaRuntime.Money.ofCents 25000)) + earned_income_local)) ((fun (income_event_local : (List IncomeEvent)) => (if ((income_event_local).length > (0 : Int)) then (match (List.map ((fun (event : IncomeEvent) => (event).earned_income)) income_event_local) with | [] => (fun () => (CatalaRuntime.Money.ofCents 0)) () | x0 :: xn => List.foldl (fun (sum1 : CatalaRuntime.Money) (sum2 : CatalaRuntime.Money) => (sum1 + sum2)) x0 xn) else (CatalaRuntime.Money.ofCents 0))) (List.filter ((fun (income_event : IncomeEvent) => (decide (((income_event).individual = taxpayer_local)) && (decide (((income_event).tax_year = (individual_tax_return).tax_year)) && (!decide ((income_event).is_counterfactual)))))) income_events)))) (get_taxpayer individual_tax_return)))) else .ok none] with | .ok none => (CatalaRuntime.Money.ofCents 300000) | .ok (some r) => r | .error _ => CatalaRuntime.Money.ofCents (-100))
  ...
  section_68_three_percent_reduction : CatalaRuntime.Money := ((fun (excess_agi : CatalaRuntime.Money) => (if (excess_agi > (CatalaRuntime.Money.ofCents 0)) then (CatalaRuntime.multiply excess_agi (Rat.mk 3 100)) else (CatalaRuntime.Money.ofCents 0))) (adjusted_gross_income - applicable_amount))
  standard_deduction : CatalaRuntime.Money := (match processExceptions [if (!decide (standard_deduction_eligible)) then .ok (some ((CatalaRuntime.Money.ofCents 0))) else .ok none] with | .ok none => (basic_standard_deduction + additional_standard_deduction) | .ok (some r) => r | .error _ => CatalaRuntime.Money.ofCents (-100))
  taxpayer_exemption_result : IndividualSection151ExemptionOutput := (taxpayer_exemption).main_output
  section_68_reduction_amount : CatalaRuntime.Money := (if (adjusted_gross_income > applicable_amount) then (Money_en.min section_68_three_percent_reduction section_68_eighty_percent_reduction) else (CatalaRuntime.Money.ofCents 0))
  itemized_deductions_after_68 : CatalaRuntime.Money := (match processExceptions [if (is_tax_year_2018_through_2025 (individual_tax_return).tax_year) then .ok (some (itemized_deductions)) else .ok none] with | .ok none => (itemized_deductions - section_68_reduction_amount) | .ok (some r) => r | .error _ => CatalaRuntime.Money.ofCents (-100))
  taxable_income : CatalaRuntime.Money := (match processExceptions [if ((fun (itemizes_deductions : Bool) => itemizes_deductions) (match (individual_tax_return).details with
  | FilingStatusVariant.JointReturn variant => (variant).itemization_election
  | FilingStatusVariant.SurvivingSpouse variant => (variant).itemization_election
  | FilingStatusVariant.HeadOfHousehold variant => (variant).itemization_election
  | FilingStatusVariant.Single variant => (variant).itemization_election
  | FilingStatusVariant.MarriedFilingSeparate variant => (variant).itemization_election)) then .ok (some (((fun (computed_taxable_income : CatalaRuntime.Money) => (if (computed_taxable_income < (CatalaRuntime.Money.ofCents 0)) then (CatalaRuntime.Money.ofCents 0) else computed_taxable_income)) ((adjusted_gross_income - itemized_deductions_after_68) - (taxpayer_exemption_result).personal_exemptions_deduction)))) else .ok none] with | .ok none => ((fun (computed_taxable_income : CatalaRuntime.Money) => (if (computed_taxable_income < (CatalaRuntime.Money.ofCents 0)) then (CatalaRuntime.Money.ofCents 0) else computed_taxable_income)) ((adjusted_gross_income - standard_deduction) - (taxpayer_exemption_result).personal_exemptions_deduction)) | .ok (some r) => r | .error _ => CatalaRuntime.Money.ofCents (-100))
  tax : CatalaRuntime.Money := (match processExceptions [if (decide (((taxpayer_marital_status).main_output).is_married_for_tax_purposes) &&
  ((match (individual_tax_return).details with
  | FilingStatusVariant.JointReturn variant => false
  | FilingStatusVariant.SurvivingSpouse variant => false
  | FilingStatusVariant.HeadOfHousehold variant => false
  | FilingStatusVariant.Single variant => false
  | FilingStatusVariant.MarriedFilingSeparate variant => true))) then .ok (some ((if (taxable_income ≤ (CatalaRuntime.Money.ofCents 1845000)) then (CatalaRuntime.multiply taxable_income (Rat.mk 3 20)) else (if (taxable_income ≤ (CatalaRuntime.Money.ofCents 4457500)) then ((CatalaRuntime.Money.ofCents 276750) + (CatalaRuntime.multiply (taxable_income - (CatalaRuntime.Money.ofCents 1845000)) (Rat.mk 7 25))) else (if (taxable_income ≤ (CatalaRuntime.Money.ofCents 7000000)) then ((CatalaRuntime.Money.ofCents 1008250) + (CatalaRuntime.multiply (taxable_income - (CatalaRuntime.Money.ofCents 4457500)) (Rat.mk 31 100))) else (if (taxable_income ≤ (CatalaRuntime.Money.ofCents 12500000)) then ((CatalaRuntime.Money.ofCents 1796425) + (CatalaRuntime.multiply (taxable_income - (CatalaRuntime.Money.ofCents 7000000)) (Rat.mk 9 25))) else ((CatalaRuntime.Money.ofCents 3776425) + (CatalaRuntime.multiply (taxable_income - (CatalaRuntime.Money.ofCents 12500000)) (Rat.mk 99 250))))))))) else .ok none, if (decide (((taxpayer_marital_status).main_output).is_married_for_tax_purposes) &&  ((match (individual_tax_return).details with
  | FilingStatusVariant.JointReturn variant => true
  | FilingStatusVariant.SurvivingSpouse variant => false
  | FilingStatusVariant.HeadOfHousehold variant => false
  | FilingStatusVariant.Single variant => false
  | FilingStatusVariant.MarriedFilingSeparate variant => false))) then .ok (some ((if (taxable_income ≤ (CatalaRuntime.Money.ofCents 3690000)) then (CatalaRuntime.multiply taxable_income (Rat.mk 3 20)) else (if (taxable_income ≤ (CatalaRuntime.Money.ofCents 8915000)) then ((CatalaRuntime.Money.ofCents 553500) + (CatalaRuntime.multiply (taxable_income - (CatalaRuntime.Money.ofCents 3690000)) (Rat.mk 7 25))) else (if (taxable_income ≤ (CatalaRuntime.Money.ofCents 14000000)) then ((CatalaRuntime.Money.ofCents 2016500) + (CatalaRuntime.multiply (taxable_income - (CatalaRuntime.Money.ofCents 8915000)) (Rat.mk 31 100))) else (if (taxable_income ≤ (CatalaRuntime.Money.ofCents 25000000)) then ((CatalaRuntime.Money.ofCents 3592850) + (CatalaRuntime.multiply (taxable_income - (CatalaRuntime.Money.ofCents 14000000)) (Rat.mk 9 25))) else ((CatalaRuntime.Money.ofCents 7552850) + (CatalaRuntime.multiply (taxable_income - (CatalaRuntime.Money.ofCents 25000000)) (Rat.mk 99 250))))))))) else .ok none, if is_surviving_spouse then .ok (some ((if (taxable_income ≤ (CatalaRuntime.Money.ofCents 3690000)) then (CatalaRuntime.multiply taxable_income (Rat.mk 3 20)) else (if (taxable_income ≤ (CatalaRuntime.Money.ofCents 8915000)) then ((CatalaRuntime.Money.ofCents 553500) + (CatalaRuntime.multiply (taxable_income - (CatalaRuntime.Money.ofCents 3690000)) (Rat.mk 7 25))) else (if (taxable_income ≤ (CatalaRuntime.Money.ofCents 14000000)) then ((CatalaRuntime.Money.ofCents 2016500) + (CatalaRuntime.multiply (taxable_income - (CatalaRuntime.Money.ofCents 8915000)) (Rat.mk 31 100))) else (if (taxable_income ≤ (CatalaRuntime.Money.ofCents 25000000)) then ((CatalaRuntime.Money.ofCents 3592850) + (CatalaRuntime.multiply (taxable_income - (CatalaRuntime.Money.ofCents 14000000)) (Rat.mk 9 25))) else ((CatalaRuntime.Money.ofCents 7552850) + (CatalaRuntime.multiply (taxable_income - (CatalaRuntime.Money.ofCents 25000000)) (Rat.mk 99 250))))))))) else .ok none, if is_head_of_household then .ok (some ((if (taxable_income ≤ (CatalaRuntime.Money.ofCents 2960000)) then (CatalaRuntime.multiply taxable_income (Rat.mk 3 20)) else (if (taxable_income ≤ (CatalaRuntime.Money.ofCents 7640000)) then ((CatalaRuntime.Money.ofCents 444000) + (CatalaRuntime.multiply (taxable_income - (CatalaRuntime.Money.ofCents 2960000)) (Rat.mk 7 25))) else (if (taxable_income ≤ (CatalaRuntime.Money.ofCents 12750000)) then ((CatalaRuntime.Money.ofCents 1754400) + (CatalaRuntime.multiply (taxable_income - (CatalaRuntime.Money.ofCents 7640000)) (Rat.mk 31 100))) else (if (taxable_income ≤ (CatalaRuntime.Money.ofCents 25000000)) then ((CatalaRuntime.Money.ofCents 3338500) + (CatalaRuntime.multiply (taxable_income - (CatalaRuntime.Money.ofCents 12750000)) (Rat.mk 9 25))) else ((CatalaRuntime.Money.ofCents 7748500) + (CatalaRuntime.multiply (taxable_income - (CatalaRuntime.Money.ofCents 25000000)) (Rat.mk 99 250))))))))) else .ok none] with | .ok none => (if (taxable_income ≤ (CatalaRuntime.Money.ofCents 2210000)) then (CatalaRuntime.multiply taxable_income (Rat.mk 3 20)) else (if (taxable_income ≤ (CatalaRuntime.Money.ofCents 5350000)) then ((CatalaRuntime.Money.ofCents 331500) + (CatalaRuntime.multiply (taxable_income - (CatalaRuntime.Money.ofCents 2210000)) (Rat.mk 7 25))) else (if (taxable_income ≤ (CatalaRuntime.Money.ofCents 11500000)) then ((CatalaRuntime.Money.ofCents 1210700) + (CatalaRuntime.multiply (taxable_income - (CatalaRuntime.Money.ofCents 5350000)) (Rat.mk 31 100))) else (if (taxable_income ≤ (CatalaRuntime.Money.ofCents 25000000)) then ((CatalaRuntime.Money.ofCents 3117200) + (CatalaRuntime.multiply (taxable_income - (CatalaRuntime.Money.ofCents 11500000)) (Rat.mk 9 25))) else ((CatalaRuntime.Money.ofCents 7977200) + (CatalaRuntime.multiply (taxable_income - (CatalaRuntime.Money.ofCents 25000000)) (Rat.mk 99 250))))))) | .ok (some r) => r | .error _ => CatalaRuntime.Money.ofCents (-100000))

def IRCSimplified_adjusted_gross_income (input : IRCSimplified_Input) : D CatalaRuntime.Money :=
  .ok (some input.adjusted_gross_income)
...
def iRCSimplified (input : IRCSimplified_Input) : IRCSimplified :=
  ...
  let taxpayer_marital_status := individualSection7703MaritalStatus { individual := (match (input.individual_tax_return).details with
  | FilingStatusVariant.JointReturn variant => (variant).taxpayer
  | FilingStatusVariant.SurvivingSpouse variant => (variant).taxpayer
  | FilingStatusVariant.HeadOfHousehold variant => (variant).taxpayer
  | FilingStatusVariant.Single variant => (variant).taxpayer
