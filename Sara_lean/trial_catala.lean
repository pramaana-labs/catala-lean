...
def individualSection151ExemptionsList (input : IndividualSection151ExemptionsList_Input) : IndividualSection151ExemptionsList :=
  let main_output := match IndividualSection151ExemptionsList_main_output_leaf_0 input with | .ok (some val) => val | _ => sorry
...
def individualSection152QualifyingChild (input : IndividualSection152QualifyingChild_Input) : IndividualSection152QualifyingChild :=
  let main_output := match IndividualSection152QualifyingChild_main_output_leaf_0 input with | .ok (some val) => val | _ => sorry
  { main_output := main_output }
...
def individualSection152QualifyingRelative (input : IndividualSection152QualifyingRelative_Input) : IndividualSection152QualifyingRelative :=
  let main_output := match IndividualSection152QualifyingRelative_main_output_leaf_0 input with | .ok (some val) => val | _ => sorry
  { main_output := main_output }
...
structure IndividualSection7703MaritalStatus_Input where
  ...
  subsection_b_exception_applies : Bool := (match processExceptions [if (decide (is_married_at_determination_date) && ((!decide (is_legally_separated)) && (decide ((section_7703_files_separate_return individual individual_tax_return)) && decide (spouse_not_member_of_household_last_6_months)))) then .ok (some (true)) else .ok none] with | .ok none => false | .ok (some r) => r | .error _ => sorry)
  is_married_for_tax_purposes : Bool := (match processExceptions [if (decide (is_married_at_determination_date) && (!decide (is_legally_separated))) then .ok (some (true)) else .ok none, if subsection_b_exception_applies then .ok (some (false)) else .ok none] with | .ok none => false | .ok (some r) => r | .error _ => sorry)
...
def individualSection7703MaritalStatus (input : IndividualSection7703MaritalStatus_Input) : IndividualSection7703MaritalStatus :=
  let main_output := match IndividualSection7703MaritalStatus_main_output_leaf_0 input with | .ok (some val) => val | _ => sorry
  { main_output := main_output }
...
def organizationSection3306EmployerStatus (input : OrganizationSection3306EmployerStatus_Input) : OrganizationSection3306EmployerStatus :=
  let main_output := match OrganizationSection3306EmployerStatus_main_output_leaf_0 input with | .ok (some val) => val | _ => sorry
  { main_output := main_output }
...
def wagePaymentEventSection3306Wages (input : WagePaymentEventSection3306Wages_Input) : WagePaymentEventSection3306Wages :=
  let main_output := match WagePaymentEventSection3306Wages_main_output_leaf_0 input with | .ok (some val) => val | _ => sorry
  { main_output := main_output }
...
def totalWages3306Calculation (input : TotalWages3306Calculation_Input) : TotalWages3306Calculation :=
  let main_output := match TotalWages3306Calculation_main_output_leaf_0 input with | .ok (some val) => val | _ => sorry
  { main_output := main_output }
...
def individualSection152Dependents (input : IndividualSection152Dependents_Input) : IndividualSection152Dependents :=
  let main_output := match IndividualSection152Dependents_main_output_leaf_0 input with | .ok (some val) => val | _ => sorry
  { main_output := main_output }
...
def employmentRelationshipEventSection3306Employment (input : EmploymentRelationshipEventSection3306Employment_Input) : EmploymentRelationshipEventSection3306Employment :=
  let main_output := match EmploymentRelationshipEventSection3306Employment_main_output_leaf_0 input with | .ok (some val) => val | _ => sorry
  { main_output := main_output }
...
structure EmployerUnemploymentExciseTaxFilerSection3301Tax_Input where
  ...
  | EmployerVariant.GeneralEmployer variant => (variant).employer
  | EmployerVariant.AgriculturalEmployer variant => (variant).employer
  | EmployerVariant.DomesticServiceEmployer variant => (variant).employer)) | .ok (some r) => r | .error _ => sorry)
...
def employerUnemploymentExciseTaxFilerSection3301Tax (input : EmployerUnemploymentExciseTaxFilerSection3301Tax_Input) : EmployerUnemploymentExciseTaxFilerSection3301Tax :=
  let main_output := match EmployerUnemploymentExciseTaxFilerSection3301Tax_main_output_leaf_0 input with | .ok (some val) => val | _ => sorry
...
def individualSection151Exemption (input : IndividualSection151Exemption_Input) : IndividualSection151Exemption :=
  let main_output := match IndividualSection151Exemption_main_output_leaf_0 input with | .ok (some val) => val | _ => sorry
  { main_output := main_output }
...
def taxpayerExemptionsList (input : TaxpayerExemptionsList_Input) : TaxpayerExemptionsList :=
  ...
  | FilingStatusVariant.Single variant => false
  | FilingStatusVariant.MarriedFilingSeparate variant => false) }
  let main_output := match TaxpayerExemptionsList_main_output_leaf_0 input taxpayer_exemptions with | .ok (some val) => val | _ => sorry
  { main_output := main_output }
...
structure IRCSimplified_Input where
  ...
  | Optional.Absent _ => false
  | Optional.Present s => (individual_is_blind_at_close s blindness_status_events death_events spouse_death_date_local)))) (if ((spouse_death_events_local).length > (0 : Int)) then (match (List.map ((fun (death_event : DeathEvent) => (death_event).death_date)) spouse_death_events_local) with | [] => (fun () => sorry /-unsupported expression-/) () | x0 :: xn => List.foldl (fun (min1 : CatalaRuntime.Date) (min2 : CatalaRuntime.Date) => (if (min1 < min2) then min1 else min2)) x0 xn) else year_end_local))) (match spouse_local with
  | Optional.Absent _ => (List.filter ((fun (death_event : DeathEvent) => false)) death_events)
  | Optional.Present s => (List.filter ((fun (death_event : DeathEvent) => (decide (((death_event).decedent = s)) && decide (((Date_en.get_year (death_event).death_date) = (individual_tax_return).tax_year))))) death_events)))) (get_year_end (individual_tax_return).tax_year))) (get_spouse individual_tax_return)) then .ok (some ((CatalaRuntime.Money.ofCents 60000))) else .ok none] with | .ok none => (CatalaRuntime.Money.ofCents 0) | .ok (some r) => r | .error _ => sorry)
  ...
  | FilingStatusVariant.MarriedFilingSeparate variant => true))))) (match spouse_local with
  | Optional.Absent _ => false
  | Optional.Present s => (individual_attained_age_65 s birth_events year_end_local)))) (get_year_end (individual_tax_return).tax_year))) (get_spouse individual_tax_return)) then .ok (some ((CatalaRuntime.Money.ofCents 60000))) else .ok none] with | .ok none => (CatalaRuntime.Money.ofCents 0) | .ok (some r) => r | .error _ => sorry)
  ...
  | FilingStatusVariant.MarriedFilingSeparate variant => (variant).itemization_election)) || decide ((extract_spouse_itemization_election individual_tax_return))))) then .ok (some (false)) else .ok none] with | .ok none => true | .ok (some r) => r | .error _ => sorry)
  ...
  | FilingStatusVariant.HeadOfHousehold variant => false
  | FilingStatusVariant.Single variant => false
  | FilingStatusVariant.MarriedFilingSeparate variant => false))) || decide (is_surviving_spouse)) then (CatalaRuntime.Money.ofCents 30000000) else (CatalaRuntime.Money.ofCents 0)) | .ok (some r) => r | .error _ => sorry)
  ...
  standard_deduction : CatalaRuntime.Money := (match processExceptions [if (!decide (standard_deduction_eligible)) then .ok (some ((CatalaRuntime.Money.ofCents 0))) else .ok none] with | .ok none => (basic_standard_deduction + additional_standard_deduction) | .ok (some r) => r | .error _ => sorry)
  ...
  itemized_deductions_after_68 : CatalaRuntime.Money := (match processExceptions [if (is_tax_year_2018_through_2025 (individual_tax_return).tax_year) then .ok (some (itemized_deductions)) else .ok none] with | .ok none => (itemized_deductions - section_68_reduction_amount) | .ok (some r) => r | .error _ => sorry)
