...
def valid := (fun (p : Period) => (if ((p)._end < (p).begin) then false else true))

def duration := (fun (p : Period) => (((CatalaRuntime.Duration.create 0 0 1) + (p)._end) - (p).begin))

def are_adjacent := (fun (p1 : Period) (p2 : Period) => ((p1)._end = ((p2).begin - (CatalaRuntime.Duration.create 0 0 1))))

def join := (fun (p1 : Period) (p2 : Period) => ({ begin := (Date_en.min (p1).begin (p2).begin), _end := (Date_en.max (p1)._end (p2)._end) } : Period))

def contained := (fun (p : Period) (d : CatalaRuntime.Date) => (decide ((p).begin ≤ d) && decide (d ≤ (p)._end)))

def to_tuple := (fun (p : Period) => ((p).begin, (p)._end))

def of_tuple := (fun (tpl : CatalaRuntime.Date × CatalaRuntime.Date) => ({ begin := tpl.1, _end := tpl.2 } : Period))

def of_tuple2 := (fun (begin_date : CatalaRuntime.Date) (_end : CatalaRuntime.Date) => ({ begin := begin_date, _end := _end } : Period))

def intersection := (fun (p1 : Period) (p2 : Period) => ((fun (intersection : Period) => (if (valid intersection) then (Optional.Present intersection) else (Optional.Absent ()))) ({ begin := (Date_en.max (p1).begin (p2).begin), _end := (Date_en.min (p1)._end (p2)._end) } : Period)))

def find_period := (fun (l : (List Period)) (d : CatalaRuntime.Date) => (List.foldl ((fun (found : (Optional Period)) (p : Period) => (match found with
...
def to_tuple_list := (fun (l : (List Period)) => (List.map ((fun (p : Period) => (to_tuple p))) l))

def to_tuple_associated_list {t1 : Type} (l : List (Period × t1)) : List ((CatalaRuntime.Date × CatalaRuntime.Date) × t1) :=
  List.map (fun (p : Period × t1) => ((to_tuple p.1), p.2)) l

def of_tuple_list := (fun (l : (List (CatalaRuntime.Date × CatalaRuntime.Date))) => (List.map (fun (tpl : (CatalaRuntime.Date × CatalaRuntime.Date)) => (of_tuple tpl)) l))

def of_tuple_associated_list {t1 : Type} (l : List ((CatalaRuntime.Date × CatalaRuntime.Date) × t1)) : List (Period × t1) :=
...
def sort_by_date {t : Type} (l : List (Period × t)) : List (Period × t) :=
