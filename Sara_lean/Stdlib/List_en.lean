...
@[simp, grind] def sequence (begin_val : Int) (end_val : Int) : List Int :=
  if begin_val > end_val then []
  else
    let length := (end_val - begin_val + 1).toNat
    (List.range length).map (fun i => begin_val + (Int.ofNat i))

@[simp, grind] def remove_first_element := (fun {t : Type} (lst : (List t)) => (remove_nth_element lst (1 : Int)))

@[simp, grind] def remove_last_element := (fun {t : Type} (lst : (List t)) => (remove_nth_element lst (lst.length : Int)))

@[simp, grind] def last_element := (fun {t : Type} (lst : (List t)) => (nth_element lst (lst.length : Int)))

@[simp, grind] def first_element := (fun {t : Type} (lst : (List t)) => (nth_element lst (1 : Int)))
